...
function shipControl.Run(delta)
	...
	if not w and not s and not e and not q then ship.Velocity = Vector3.New(ship.Velocity.x,0,ship.Velocity.z)/1.1 end
...
function shipControl.inputBegin(inputobject) 
    if tostring(inputobject.InputKeyCode) == "W" and tostring(inputobject.InputState) == "Begin" then 
		w = true 
		ship.Engine1.Enable = true
		ship.Engine2.Enable = true
	end
    if tostring(inputobject.InputKeyCode) == "LeftControl" and tostring(inputobject.InputState) == "Begin" then 
		c = true 
		if w then
			ship.Booster1.Enable = true
			ship.Booster2.Enable = true
		end
	end
...
function shipControl.inputEnd(inputobject) 
	if tostring(inputobject.InputKeyCode) == "W" and tostring(inputobject.InputState) == "End" then 
		w = false 
		ship.Engine1.Enable = false
		ship.Engine2.Enable = false
		ship.Booster1.Enable = false
		ship.Booster2.Enable = false
	end
	if tostring(inputobject.InputKeyCode) == "LeftControl" and tostring(inputobject.InputState) == "End" then 
		c = false 
		ship.Booster1.Enable = false
		ship.Booster2.Enable = false
	end
